...
hist(data$pain_cat, main = "Histogram of Pain catastrophizing", xlab = "pain_cat", ylab = "Frequency")
...
```{r}
updated_model <- lm(pain ~ age + sex + STAI_trait + pain_cat + mindfulness + cortisol_serum, data = data)
...
Furthermore the regression equation of the more complex model was the following: pain = 2.576 − 0.034 * age - 0.316 * sex - 0.013 * STAI_trait + 0.083 * pain_cat − 0.136 * mindfulness + 0.543 * cortisol_serum. The regression model was statistically significant, F(6, 151) = 12.85, p < .001, and explained 33.8% of the variance in pain scores (R² = .338, Adjusted R² = .312). The residual standard error was 1.253.
Pain catastrophizing (B = 0.083, SE = 0.028, t(151) = 2.926, p = .004) and the cortisol level in blood (B = 0.543, SE = 0.125, t(151) = 4.351, p < .001) were significant predictors of pain.

```{r}
tab_model(updated_model, show.se = TRUE, show.stat = TRUE, show.aic = TRUE)
```

The complex model explained significantly more (25.8 %) variability than the simple model.
...
In our study we found that pain catastrophizing and the cortisol level in the bloodstream  were significant predictors of pain. The conclusion is that the higher the cortisol level in the bloodstream the higher the pain experienced around and after surgeries by the person. We also found, that the pain catastrophizing questionnaire is a reliable indication of the experienced pain.
